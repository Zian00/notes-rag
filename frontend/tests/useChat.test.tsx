...
describe("useChat", () => {
  ...
  it("invalidates this conversation's detail query after done", async () => {
    // The turn just changed this conversation's server-side history, but the
    // cached detail still holds the pre-question version. With staleTime=30s the
    // cache is served without a refetch, so navigating away and back re-seeds
    // from it and the last Q&A silently disappears until a full page reload.
    mockStreamChat.mockImplementation(
      scriptedStream([
        { event: "meta", data: { conversation_id: "convo-1" } },
        { event: "token", data: { delta: "ok" } },
    ...
    await waitFor(() => expect(result.current.isStreaming).toBe(false))
    expect(invalidateSpy).toHaveBeenCalledWith(
      expect.objectContaining({ queryKey: getConversationDetailKey("convo-1") })
    )
  })
